...
class ChatService:
    def __init__(self, store, session, emit, clock=None):
        self.store = store
    ...
    def _estimate(messages: list[dict]) -> int:
        ...
        return 256 + sum(len(message["content"].encode("utf-8")) + 32 for message in messages)

    def send(self, conversation_id: str | None, text: str, max_tokens: int = 2048) -> dict:
        ...
        if conversation_id:
            ...
            history = []
        request = [{"role": message["role"], "content": message["text"]}
        ...
        metadata = {"config": config, "session_id": snapshot.get("session_id"),
                    ...
                    "prompt_estimate_basis": "UTF-8 bytes + 32/message + 256 template reserve",
