...
Дата: 24 сентября 2026. Статус: проект архитектуры, до реализации.
...
Этот документ описывает целевое устройство приложения. Упомянутые ниже новые модули и команды ещё не созданы. Изменения ограничены документацией.
...
Стрелки показывают направление вызовов. Состояние возвращается неизменяемыми снимками и типизированными событиями. Сборка зависимостей находится в одном `bootstrap.py`: он создаёт объекты и явно передаёт их друг другу.
...
Целевая карта; файлы добавляются по мере появления сценариев, без пустого каркаса заранее:

```text
src/model_studio/
  bootstrap.py           создание зависимостей и запуск desktop
  domain.py              общие типы конфигурации, сессии, метрик и ошибок
  catalog.py             установки, идентичность, профиль, безопасное удаление
  configuration.py       проверка и разрешение желаемых параметров
  session.py             жизненный цикл и занятость сессии
  chat.py                отправка и сохранение бесед
  benchmarks/
    runner.py            единая процедура измерения
    research.py          ограниченный план и продвижение исследования
    recommendations.py   выбор режимов с объяснением
    reports.py           текст/JSON одного результата
    workloads/           версионированные тестовые нагрузки
  backends/
    contract.py          общий потоковый запрос и способности backend
    transport.py         HTTP, поток, таймауты и отмена
    ollama.py            протокол Ollama
    llama_cpp.py         протокол llama.cpp и его метрики
    process.py           принадлежащий приложению сервер
  platform/
    paths.py             системные каталоги и ресурсы
    windows_process.py   владение деревом процесса
    telemetry.py         доступные измерители компьютера
  storage/
    database.py          соединения, транзакции, миграции, backup
    catalog.py           запросы моделей и установок
    history.py           результаты и исследования
    chat.py              диалоги и сообщения
    preferences.py       проекты и настройки
    legacy_import.py     однократный импорт старых данных
    migrations/          последовательные изменения схемы
  integrations/
    opencode.py          подготовка подключения и запуск клиента
  desktop/
    controllers.py       команды страниц, без бизнес-алгоритмов
    models.py            Qt-модели списков и снимков
    workers.py           выполнение задач и доставка событий в Qt
    qml/
      Main.qml           оболочка и общая строка сессии
      Theme.qml          цвета, типографика, интервалы и размеры
      components/        повторяющиеся визуальные элементы
      pages/             Launch, Chat, Models, Research, Settings
tests/
docs/
  ARCHITECTURE.md
  design-v2/
...
## 13. Переход от текущего приложения
...
## 15. Проверка архитектуры по audit-project
